...
const path='public/game/east-district/v1/east-district.glb';
...
const metrics=JSON.parse(fs.readFileSync('output/east-district/metrics.json'));
metrics.compressedBytes=fs.statSync(path).size;fs.writeFileSync('output/east-district/metrics.json',JSON.stringify(metrics,null,2));console.log(metrics);
