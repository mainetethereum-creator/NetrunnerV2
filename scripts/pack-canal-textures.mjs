...
const path='public/game/canal/v1/canal-kit.glb';
...
await sharp('output/canal/references/water-normal.png').resize(1024,1024).webp({lossless:true,effort:5}).toFile('public/game/canal/v1/water-normal.webp');
const metrics=JSON.parse(fs.readFileSync('output/canal/metrics.json'));
metrics.compressedBytes=fs.statSync(path).size;metrics.waterNormalBytes=fs.statSync('public/game/canal/v1/water-normal.webp').size;
fs.writeFileSync('output/canal/metrics.json',JSON.stringify(metrics,null,2));console.log(metrics);
