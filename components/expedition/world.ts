...
import {TEST_PATCH,TEST_TREES} from '../vegetation/format.ts';
...
export const TREES=makeTrees(STATIC_COLLIDERS).filter(t=>Math.abs(t.x-TEST_PATCH.x)>TEST_PATCH.w/2+1||Math.abs(t.z-TEST_PATCH.z)>TEST_PATCH.d/2+1);
const WORLD_COLLIDERS=[...STATIC_COLLIDERS,...TEST_TREES.map(t=>({x:t.x,z:t.z,w:.65,d:.65})),...TREES.map(t=>({x:t.x,z:t.z,w:.8*t.scale,d:.8*t.scale}))];
