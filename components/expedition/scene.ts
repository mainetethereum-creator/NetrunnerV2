...
export function createExpedition(host:HTMLElement,onState:(s:Snapshot)=>void,onError:(s:string)=>void) {
  const session=new ExpeditionSession(); let debug=false,autoFire=false;
  const world=makeWorld(),scene=new T.Scene();scene.background=new T.Color('#0c161d');scene.fog=new T.FogExp2('#0c191e',.018);
  ...
  function fire(){const target=session.attack(player.position);if(target){const points=beamGeo.attributes.position as T.BufferAttribute;points.setXYZ(0,player.position.x,1.3,player.position.z);points.setXYZ(1,target.x,1.2,target.z);points.needsUpdate=true;beamGeo.computeBoundingSphere();beam.visible=true;beamUntil=performance.now()+110;}}
  ...
  function animate(now:number) {
    ...
    session.tick(dt,player.position);if(autoFire)fire();
    env.stream(player.position,debug,session.events.has('drop')&&!session.opened.has('drop'));
    const visibleEnemies=session.enemies.filter(e=>e.hp>0&&Math.hypot(e.x-player.position.x,e.z-player.position.z)<30);
    enemyPool.forEach((body,i)=>{const enemy=visibleEnemies[i];body.visible=!!enemy;if(enemy){body.position.set(enemy.x,enemy.elite?1.35:1,enemy.z);body.scale.setScalar(enemy.elite?1.5:1);body.rotation.y=Math.atan2(player.position.x-enemy.x,player.position.z-enemy.z);}});
