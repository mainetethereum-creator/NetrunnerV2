// Adapted from GrassSystemThreeJS (MIT); see LICENSE.
import * as THREE from 'three';
type Uniforms=Record<string,THREE.IUniform>;
type GrassOptions={sharedUniforms:Uniforms;soilUniforms:Uniforms;mossUniforms:Uniforms;windUniforms:Uniforms;noiseGLSL:string;heightGLSL:string;sunLight:THREE.Object3D;maxCount?:number;area?:number;segments?:number};
...
export function createGrass({
 ...
 attribute float iGround;
        ...
        void computeBlade() {
          ...
          float gy = iGround;          // snap base to the terrain
          gPos = vec3(iPos.x + pR.x + windOff.x,
                      gy + pR.y,
                      iPos.y + pR.z + windOff.y);
          if(iHeight <= 0.0) gPos = vec3(iPos.x, gy, iPos.y);
          gNrm = normalize(nR);
          vT = t;
          vColorVar = iColorVar;
          vGrassWorld = gPos;
