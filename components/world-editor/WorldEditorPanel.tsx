...
export default function WorldEditorPanel({editor,unlockedMap=false}:{editor:WorldEditor;unlockedMap?:boolean}){
 ...
 <p>{unlockedMap ? 'Доступны здания, стены, пол, декор и NPC. Составные объекты выбираются целиком. Delete — удалить; Ctrl+Z — вернуть. Герой и логические границы карты не редактируются.' : 'LOCKED: объединённые полы, terrain, статическая оболочка, игровые триггеры и противники.'}</p>
 ...
 <div><button disabled={!state.canSave} onClick={()=>editor.save()}>Сохранить</button><button disabled={state.loading} onClick={()=>editor.load()}>Загрузить</button><button onClick={()=>editor.reset()}>Исходная карта</button></div>
 <details><summary>JSON экспорт / импорт</summary><p>Отдельный документ этой карты. Импорт можно отменить. Максимум 500 КБ.</p><button disabled={!state.canSave} onClick={()=>{const text=editor.exportJSON();setJson(text);const url=URL.createObjectURL(new Blob([text],{type:'application/json'})),a=document.createElement('a');a.href=url;a.download='world-map.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}}>Экспорт JSON</button><textarea aria-label="JSON карты" value={json} onChange={e=>setJson(e.target.value)} maxLength={500001}/><input aria-label="Импорт файла JSON" type="file" accept="application/json,.json" onChange={async e=>{const file=e.target.files?.[0];if(file){if(file.size>500000){setJson('Файл превышает 500 КБ');return;}setJson(await file.text());}}}/><button onClick={()=>editor.importJSON(json)}>Применить JSON</button></details>
