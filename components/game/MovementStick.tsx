...
export default function MovementStick({onMove,disabled=false}:{onMove:(x:number,z:number)=>void;disabled?:boolean}) {
  ...
  return <div ref={node} className={styles.stick} data-disabled={disabled} role="group" aria-label="Movement stick: hold and drag in any direction" onContextMenu={e=>e.preventDefault()} onPointerDown={e=>{if(disabled||active.current!==null||document.querySelector('[aria-modal="true"]'))return;e.preventDefault();active.current=e.pointerId;e.currentTarget.setPointerCapture(e.pointerId);update(e);}} onPointerMove={e=>{if(active.current===e.pointerId)update(e);}} onPointerUp={e=>{if(active.current===e.pointerId){reset();if(e.currentTarget.hasPointerCapture(e.pointerId))e.currentTarget.releasePointerCapture(e.pointerId);}}} onPointerCancel={e=>{if(active.current===e.pointerId)reset();}} onLostPointerCapture={e=>{if(active.current===e.pointerId)reset();}}><span/><small>MOVE</small></div>;
}
