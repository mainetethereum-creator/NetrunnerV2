...
export function buildRefugeZones({ box, cylinder, pipe, sign, light, m, surface }: ArchitectureTools) {
  ...
  for (let x = -27; x < 28; x += 3.4) {
    box(x, .1, -21, 3.32, 2.4, .5, m.dark);
    box(x, 1.35, -21, 3.42, .14, .66, m.edge);
    box(x - 1.65, .15, -21, .18, 2.6, .8, m.edge);
  }
  ...
  for (const [x, z] of [[-3, 4.3], [6.8, 6.9]]) {
