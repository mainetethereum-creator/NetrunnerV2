...
export const COLLIDERS: Rect[] = [
  ...
  { x: BASE_MEDIA_TOWER.x, z: BASE_MEDIA_TOWER.z, w: BASE_MEDIA_TOWER.width, d: BASE_MEDIA_TOWER.depth },
];
...
const authoredColliderOverrides = new Map<number, Rect[]>();
export function setBaseAuthoredCollider(index: number, rects: Rect[] | null) {
  if (rects === null) authoredColliderOverrides.delete(index);
  else authoredColliderOverrides.set(index, rects);
}
...
export function clearBaseEditor(){editorColliders=[];stationOverrides.clear();authoredColliderOverrides.clear();}
