...
export default function Metro3D(){
  ...
  return <main className={styles.root}>
    ...
    {!hidden&&<>
      <header className={styles.header}><div><span className={styles.symbol}>C◇B</span><strong>CYBERBASE</strong><small>THE LOWER LINES</small></div><Link href='/'>↑ Return to refuge</Link></header>
      ...
      {snap.ready&&snap.near==='exit'&&<div className={styles.action}>{level<4?<button onClick={()=>change(level+1)}>↓ Descend to sector 0{level+1}</button>:<Link href='/'>↑ Extract to refuge</Link>}</div>}
    ...
    {error&&<div className={styles.loading}><h2>Connection interrupted</h2><p>{error}</p><Link href='/'>Return to refuge</Link></div>}
  </main>;
}
