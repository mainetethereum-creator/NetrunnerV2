...
import {makeWorld,findRoute,VEGETATION_TREES} from '../components/expedition/world.ts';
import {POIS,EXTRACTIONS} from '../components/expedition/config.ts';
...
test('full-zone trees stay off the highway and preserve every destination route',()=>{
 const world=makeWorld();assert.equal(VEGETATION_TREES.length,46);
 for(const tree of VEGETATION_TREES){assert.ok(tree.z<=28||tree.z>=44);assert.equal(world.canStand(tree),false);assert.ok([...POIS,...EXTRACTIONS].every(p=>Math.hypot(tree.x-p.x,tree.z-p.z)>=3));}
 for(const destination of [...POIS,...EXTRACTIONS])assert.ok(findRoute(world,world.spawn,destination).length,destination.id);
});
